		...
		<div class="block-area-content">
			<div class="block-area-text">
				<?php if ( ! empty( $headline ) ) { ?>
					<h2 class="block-area-title font-weight-600"><?php echo esc_html( $headline ); ?></h2>
				<?php } ?>
				<?php if ( ! empty( $text ) ) { ?>
					<div class="block-area-paragraph restricted-width"><?php echo wp_kses_post( $text ); ?></div>
				<?php } ?>
				<?php if ( ! empty( $button_text ) && ! empty( $url ) ) { ?>
					<a class="block-area-button-link" href="<?php echo esc_url( $url ); ?>"><button class="block-area-button button-arrow-right"><?php echo esc_html( $button_text ); ?></button></a>
				<?php } ?>
			</div>
		</div>
	</div>
