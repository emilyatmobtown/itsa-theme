<?php
/**
 * This file contains functions related to the custom settings page.
 *
 * @package ITSATheme
 */

namespace ITSATheme\Settings;
...
function get_global_settings( $set_default = false ) {
	$settings = get_option( 'itsa_settings', [] );
...
function create_settings_page() {
	add_options_page(
		...
		__( 'ITSA Custom Settings', 'itsa-theme' ),
...
function do_settings_page() {
	...
	<div class="wrap">
		<h1>
			...
			esc_html_e( 'ITSA Custom Settings', 'itsa-theme' );
			?>
		</h1>
		<form action="options.php" method="post">
			<?php settings_fields( 'itsa-settings' ); ?>
			<?php do_settings_sections( 'itsa-settings' ); ?>
			<?php submit_button(); ?>
		</form>
...
function text_field_callback( $args ) {
	...
	$placeholder  = __( 'Default', 'itsa-theme' );
	$placeholder .= ': ';
...
function validate_item_total( $data = null, $label = '' ) {
	...
	if ( empty( $data ) && '0' !== $data ) {
		...
		$message .= __( 'Invalid total. The default value will be used. Next time, please use a whole number greater than zero.', 'itsa-theme' );
		...
		$message .= __( 'The image total is too high. The default value will be used. Next time, please use a number that is 50 or less.', 'itsa-theme' );
...
function validate_image_dimension( $data = null, $label = '' ) {
	...
	if ( empty( $data ) && '0' !== $data ) {
		...
		// translators: This is the error message for an invalid image dimension.
		$message .= __( 'Invalid dimension. The default value will be used. Next time, please use a whole number greater than zero. To resize previously uploaded images, regenerate the thumbnails.', 'itsa-theme' );
		...
		// translators: This is the error message for an image dimension that is 9999.
		$message .= __( 'Invalid dimension. The default value will be used. Next time, please use a number that is not 9999. To resize previously uploaded images, regenerate the thumbnails.', 'itsa-theme' );
...
function validate_label( $data = null, $label = '' ) {
	...
	if ( empty( $data ) && '0' !== $data ) {
		...
		$message .= __( 'Invalid text label. The default value will be used.', 'itsa-theme' );
